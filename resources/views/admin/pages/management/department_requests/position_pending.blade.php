...
@section('scripts')
    ...
    <script>
        $(document).ready(function () {
            $('#position_list_pending').DataTable({
                pageLength: 10,
                responsive: true,
        ...
        $(document).ready(function () {
            ...
            $('#bulk_section_btn_approve').click(function () {
                ...
                var ids = [];
                var $checkedBoxes = $('#position_list_pending input[type=checkbox]:checked').not('.select_all');
            ...
            <div class="col-lg-12">
                ...
                <div class="ibox ">
                    <div class="ibox-title">
                        <h5 data-url="#">Positions Requests</h5>


                    </div>
                    <div class="ibox-content">
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover" id="position_list_pending" style="width: 100%">
                                <thead>
                                <tr>
                                ...
                                <tbody>
                                @foreach($requested_positions as $position)
                                    <tr>
                                        <td><input type="checkbox"  name="row_id" id="checkbox_{{$position->id}}" value="{{$position->id}}"></td>
                                        ...
                                        <td>{{$position->department ? $section->department->department_short_name : ''}}</td>
                                        <td>{{$position->created_at}}</td>
                                        <td>{{$position->is_approved ? "Approved" : "Pending"}}</td>
                                    </tr>
                                @endforeach
        ...
        <div class="modal-dialog">
            <div class="modal-content animated fadeIn">
                <div class="modal-header" style="text-align: center">
                    ...
                    <small>Position Confirmation</small>
                </div>
                <div class="modal-body">
                    <p><strong>Are you sure you want to Approve these positions?</strong></p>
                </div>
                <div class="modal-footer">
                    <form action="{{url('admin/position_requests/position_pending_status')}}" id="bulk_section_form" method="POST">
                        @csrf
