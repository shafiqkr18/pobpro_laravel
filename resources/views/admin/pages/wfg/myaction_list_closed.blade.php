                    ...
                    <div class="ibox-content">
                            ...
                            <table class="table table-striped table-bordered table-hover " id="wfg_process_list" style="width: 100%">
                                ...
                                @if(count($lists) > 0)
                                    @foreach($lists as $res)
                                        ...
                                        //process
                                        $str_ID_PROCESS_INST1=$res['ID_PROCESS_INST'];
                                        $str_ID_ACTIVITY_INST=$res['ID_ACTIVITY_INST'];
                                        $str_ID_APPLICATION=$res['ID_APPLICATION'];

                                        $str_closeurl = "http://8.209.76.37/wfgen/show.aspx?QUERY=ACTIVITY_INSTANCE_FORM&ID_PROCESS_INST=".$str_ID_PROCESS_INST1."&ID_ACTIVITY_INST=".$str_ID_ACTIVITY_INST."&ID_APPLICATION=".$str_ID_APPLICATION;

                                        //change $str_openurl to Base64 string
                                        $base64_str_openurl=base64_encode($str_closeurl);
                        ...
                        $username=Auth::user()->id;
                                        ...
                                        $base64_username=base64_encode($username);  //value like d2ZnZW5fYWRtaW4=
                                         ...
                                         $goto_wfg1="http://8.209.76.37/wfgen/GoToWFG.aspx?username=".$base64_username."&wfg_url=".$base64_str_openurl;
                                        @endphp
                                        <tr>

                                            <td><a href="{{$goto_wfg}}" target="_blank">{{$res['ID_PROCESS_INST']}}</a> </td>
                                            <td>{{$res['PNAME']}}</td>
                                            <td><a href="{{$goto_wfg1}}" target="_blank">{{$res['NAME']}}</a></td>
                                            {{--                                            <td>{{$res['NAME']}}</td>--}}
                                            {{--                                            <td>{{$res['DESCRIPTION']}}</td>--}}
                                            <td>{{$res['DATE_OPEN']?date('Y-m-d H:i:s',strtotime($res['DATE_OPEN'])):''}}</td>
                                            <td>{{$res['DATE_LIMIT']?date('Y-m-d H:i:s',strtotime($res['DATE_CLOSE'])):''}}</td>
