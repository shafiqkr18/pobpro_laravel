...
<div class="row wrapper page-heading">
	<div class="col-lg-10">
		...
		<h2 class="d-flex align-items-center">{{ $section->short_name }}</h2>
	</div>
			...
			<div class="ibox ">
				...
				<div class="ibox-content">
					<form role="form" id="frm_section" enctype="multipart/form-data">
						...
						<input type="hidden" name="listing_id" value="{{ $is_update ? $section->id : '' }}">
						...
						<div class="row">
								...
								<div class="form-group form-inline">
									...
									<select name="org_id" id="org_id" class="form-control form-control-sm b-r-xs">
										...
										@foreach($organizations as $key=>$val)
											<option value="{{ $val->id}}" {{ $section->org_id == $val->id ? 'selected' : '' }}>{{$val->org_title}}</option>
										@endforeach
									</select>
								</div>
								<div class="form-group form-inline">
							...
							<div class="col-sm-6">
								<div class="form-group form-inline">
									...
									<input type="text" class="form-control form-control-sm" id="section_code" name="section_code" value="{{ $section->section_code }}">
								</div>
								<div class="form-group form-inline">
									<label>Short Name</label>
									<input type="text" class="form-control form-control-sm" id="short_name" name="short_name" value="{{ $section->short_name }}">
								</div>

								<div class="form-group form-inline">
									<label>Full Name</label>
									<input type="text" class="form-control form-control-sm" id="full_name" name="full_name" value="{{ $section->full_name }}">
								</div>
							</div>
						</div>
					</form>
